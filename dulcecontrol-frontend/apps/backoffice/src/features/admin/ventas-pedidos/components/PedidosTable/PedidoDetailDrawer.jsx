    ...
    return (
        ...
        >
            {loading ? (
                ...
                <Empty description="Selecciona un pedido para ver su detalle" />
            ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    <div>
                        ...
                        <Descriptions bordered size="small" column={1}>
                            ...
                            <Descriptions.Item label="Fecha de Entrega Pactada">
                                {pedido.fechaEntregaPactada ? dayjs(pedido.fechaEntregaPactada).format('DD/MM/YYYY HH:mm') : '-'}
                            </Descriptions.Item>
                            <Descriptions.Item label="Cliente">{clienteNombre || 'Cliente general'}</Descriptions.Item>
                            <Descriptions.Item label="Tipo de Entrega">
                                {pedido.tipoEntrega?.replace('_', ' ')?.toUpperCase() || '-'}
                            </Descriptions.Item>
                            ...
                            <Descriptions.Item label="Estado del Pedido">
                                <Tag color="blue">{pedido.estadoPedido?.replace('_', ' ')?.toUpperCase()}</Tag>
                            </Descriptions.Item>
                            <Descriptions.Item label="Estado del Pago">
                                <Tag color="green">{pedido.estadoPago?.replace('_', ' ')?.toUpperCase()}</Tag>
                            </Descriptions.Item>
                            <Descriptions.Item label="Total">{formatMoney(pedido.totalFinalCentimos ?? 0)}</Descriptions.Item>
                            <Descriptions.Item label="Monto Pagado">{formatMoney(pedido.montoPagadoCentimos ?? 0)}</Descriptions.Item>
